...
use pubsub_core::{
    Message, PubSubError, Subject, SubscriptionId, SubscriptionSender, TaggedMessage,
};
use pubsub_broker::{Router, SubscriptionRegistry};
use pubsub_protocol::Frame;
...
pub struct Session {
    ...
    framed: futures::stream::SplitStream<
        tokio_util::codec::Framed<tokio::net::TcpStream, pubsub_protocol::PubSubCodec>,
    >,
...
impl Session {
    ...
    async fn handle_frame(&mut self, frame: Frame) -> Result<(), PubSubError> {
        match frame {
            Frame::Publish {
                topic,
                payload,
            ...
            other => Err(PubSubError::Protocol(format!(
    ...
    async fn handle_unsubscribe(&mut self, client_sid: u64) -> Result<(), PubSubError> {
