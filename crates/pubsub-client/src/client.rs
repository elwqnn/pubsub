...
use pubsub_core::{Message, PubSubError, Subject};
use pubsub_protocol::{Frame, PubSubCodec};
...
/// Async client for the pubsub message broker.
///
...
impl Client {
    ...
    pub async fn connect(options: ConnectOptions) -> Result<Self, PubSubError> {
        ...
        let framed = Framed::new(stream, PubSubCodec::new(options.max_frame_size));
    ...
    pub async fn request(
        ...
        match result {
            Ok(Some(msg)) => Ok(msg),
            Ok(None) => Err(PubSubError::ConnectionClosed),
            Err(_) => Err(PubSubError::Timeout),
        }
    }

    /// Subscribe to a subject pattern. Returns a stream of messages.
    pub async fn subscribe(&self, subject: &str) -> Result<SubscriptionStream, PubSubError> {
    ...
    pub async fn subscribe_queue(
        ...
        subject: &str,
        queue_group: &str,
    ) -> Result<SubscriptionStream, PubSubError> {
    ...
    pub async fn unsubscribe(&self, sid: u64) -> Result<(), PubSubError> {
    ...
    async fn send(&self, frame: Frame) -> Result<(), PubSubError> {
        self.writer_tx
            .send(frame)
            .await
    ...
    async fn writer_loop(
        mut sink: futures::stream::SplitSink<Framed<TcpStream, PubSubCodec>, Frame>,
    ...
    async fn reader_loop(
        mut stream: futures::stream::SplitStream<Framed<TcpStream, PubSubCodec>>,
        subscriptions: Arc<DashMap<u64, mpsc::Sender<Message>>>,
        shutdown: CancellationToken,
